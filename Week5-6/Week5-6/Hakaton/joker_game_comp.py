...
def bets(my_game):
    turn = my_game.diller
    if turn == 2:
        player2.bet = player2.get_hearts()
        player3.bet = player3.get_hearts()
        player4.bet = player4.get_hearts()
        if my_game.num_distribution-(player2.bet+player3.bet+player4.bet) >= 0:
            print(f'Your turn. Your cards: {player1.hand}')
            banned_bet = my_game.num_distribution - (player1.bet+player2.bet+player3.bet+player4.bet)
            player1.bet = banned_bet
            while player1.bet == banned_bet:
                player1.bet = int(input(f'Make your bet. You cant make bet of {banned_bet}: '))
        else:
            print(f'player1 turn. Your cards: {player1.hand}')
            player1.bet = int(input('make your bet: '))     
    else:
        for _ in range(3):
            if turn == 1:
                print(f'player1 turn. Your cards: {player1.hand}')
...
def step1player(my_game, player1, player2, player3, player4):
        ...
        if current_player == player1:
            ...
            mychoise = int(input('Choose one: '))
        else:
            cards = []
            for k in current_player.hand.keys():
                cards.append(k)
            mychoise = random.choice(cards)
        my_game.table[my_game.players_turn] = current_player.hand[mychoise]
        my_game.current_suit = current_player.hand[mychoise][0]
        del current_player.hand[mychoise]
        ...
        if current_player == player1:
            ...
            if my_game.current_suit in current_player.suits:
                while choise_suit != my_game.current_suit:
                    print(f'{current_player.name} turn.\n Your cards: {current_player.hand}.')
                    mychoise = int(input(f'Choose one card of current suit ({my_game.current_suit}): '))
                    choise_suit = current_player.hand[mychoise][0]
            elif 'hearts' in current_player.suits:
                while choise_suit  != 'hearts':
                    print(f'{current_player.name} turn.\n your cards: {current_player.hand}.')
                    mychoise = int(input(f'Choose one card of trump suit (heart): '))
                    choise_suit = current_player.hand[mychoise][0]
            else:
                print(f'{current_player.name} turn.\n your cards: {current_player.hand}.')
                mychoise = int(input(f'Choose any card: '))
        else:
            current_player.suits=current_player.get_suits()
            choise_comp_suit = []
            if my_game.current_suit in current_player.suits:  
                for k,v in current_player.hand.items():
                    if v[0] == my_game.current_suit:
                        choise_comp_suit.append(k)
                mychoise = random.choice(choise_comp_suit)
            elif 'hearts' in current_player.suits:
                for k,v in current_player.hand.items():
                    if v[0] == 'hearts':
                        choise_comp_suit.append(k)
